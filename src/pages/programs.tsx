...
const UndergradPrograms: React.FC = () => {
  ...
  const fetchPrograms = async (page: number) => {
    try {
      ...
      const response = await callAPI
        .from('programs')
        .select('name, is_certificate, campus')
      ...
      setPrograms(
        response.data.map((program: { name: string; is_certificate: boolean; campus: string }) => ({
          name: program.name,
          isCertificate: program.is_certificate,
  ...
  const handleNavigateProgram = (program: Program) => {
    ...
    <>
      ...
      <div className="hero bg-white shadow-md rounded-lg min-h-screen">
        <div className="container mx-auto px-4 py-6">
          <h1 className="text-2xl font-bold text-gray-800 mb-4">Undergraduate Programs</h1>
            ...
            {programs.map((program, index) => (
              ...
              >
                ...
                <p className="text-gray-700">
                  {program.isCertificate ? 'Undergraduate Certificate' : 'Undergraduate Degree'}
                </p>
...
export default UndergradPrograms;
